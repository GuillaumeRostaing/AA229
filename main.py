...
#initiate problem
gamma = 0.95
c = 10 #UCB exploration

#simulation parameters
nb_iter = 500 #nb iterations
depth = 4 #depth
nb_simulations = 1
max_reward = -50
min_reward = 0
...
for nb_simu in range(nb_simulations):
    ...
    rock_goodness = [1]
    #rock_goodness = [1,0,1]#,0]
    #rock_goodness = [1, 0, 0, 1, 1]
    #rock_position = [[0, 4], [2, 2], [2, 4], [3, 1], [3, 4]]
    ...
    belief_goodness = FRS.belief_state
    belief_position = FRS.belief_position
    total_reward = 0
    end = 0
    i = 0
    policy = []
    while (end == 0) and (i < 100):
        i += 1
        ...
        MCTS = fsmcts.FSMCTS()
        MCTS.InitiateParameters(grid_size,agents,init_agents_position,init_agents_action,agents_action,len(rock_position),c,gamma)
        next_action = MCTS.SelectAction(blf_goodness,blf_position,pos,depth,nb_iter,'legal')
        ...
        for agent_id in range(FRS.nb_agents):
            ...
            if isinstance(action,int) == True:
                action = [action]
            result = FRS.TakeAction(next_action,agent_id)
